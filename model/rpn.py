...
class SiameseRPN:
    ...
    def build_model(self, template_shape=(127, 127, 3),
        ...
        detection_cls_feature = Conv2D(detection_channel_num, kernel_size=3, activation='relu', kernel_initializer=RandomNormal(stddev=0.005),
                                       name='RPN_Detection_CLS_Conv')(detection_feature)
        detection_reg_feature = Conv2D(detection_channel_num, kernel_size=3, activation='relu', kernel_initializer=RandomNormal(stddev=0.005),
                                       name='RPN_Detection_REG_Conv')(detection_feature)

        template_cls_feature = Conv2D(filters=2 * self.anchor_num * detection_channel_num, kernel_size=3, activation='relu', kernel_initializer=RandomNormal(stddev=0.005),
                                      name='RPN_Template_CLS_Conv')(template_feature)
        template_reg_feature = Conv2D(filters=4 * self.anchor_num * detection_channel_num, kernel_size=3, activation='relu', kernel_initializer=RandomNormal(stddev=0.005),
